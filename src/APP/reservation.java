...
public class reservation extends JFrame {
	...
	public reservation() {
		...
		JLabel lblNewLabel = new JLabel("Gestion de Reservations");
		...
		lblReservation.setFont(new Font("Tahoma", Font.BOLD, 17));
		...
		JLabel lblAdulte = new JLabel("Date de R\u00E9servation");
		lblAdulte.setFont(new Font("Tahoma", Font.BOLD, 17));
		lblAdulte.setBounds(12, 308, 195, 29);
		contentPane.add(lblAdulte);
		
		JLabel lblChambre = new JLabel("Id Cat\u00E9gorie");
		lblChambre.setFont(new Font("Tahoma", Font.BOLD, 17));
		lblChambre.setBounds(12, 543, 195, 29);
		contentPane.add(lblChambre);
		
		JLabel lblFinDeReservation = new JLabel("Date D\u00E9part");
		lblFinDeReservation.setFont(new Font("Tahoma", Font.BOLD, 17));
		lblFinDeReservation.setBounds(12, 249, 195, 29);
		contentPane.add(lblFinDeReservation);
		
		JLabel lblC = new JLabel("Crit\u00E8re");
		lblC.setFont(new Font("Tahoma", Font.BOLD, 13));
		lblC.setBounds(856, 117, 56, 16);
...
//		contentPane.add(btnEnregistrer);
		...
		btnSupprimer.addActionListener(new ActionListener() {
			...
			if( id.trim().length()==0 
					|| nombres_personnes.trim().length()==0 
					|| dateArriver.trim().length()==0 
					|| dateDepart.trim().length()==0 
					|| date_reservation.trim().length()==0  
					|| statut.trim().length()==0
				    || id_client.trim().length()==0 
				    ||id_categorie.trim().length()==0 
				    || num_chambres.trim().length()==0) {
				...
				
			{
				try {
					...
					JOptionPane.showMessageDialog(null,"Reservation Effacer");
		...
		btnSupprimer.setBounds(963, 685, 131, 41);
		contentPane.add(btnSupprimer);
		
		
		
		JLabel lblEnfant = new JLabel("Nombres de Personnes");
		lblEnfant.setFont(new Font("Tahoma", Font.BOLD, 17));
		...
		lblNumchambres.setFont(new Font("Tahoma", Font.BOLD, 17));
		...
		btnModifier.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				String dateDepart = ((JTextField)dateD.getDateEditor().getUiComponent()).getText();
				String date_reservation = ((JTextField)dateR.getDateEditor().getUiComponent()).getText();
				String status = comboBoxstatut.getSelectedItem().toString();
				String id_client = txtIdClient.getText();
				String id_categorie = txtIdc.getText();
				String num_chambres =txtNc.getText();
...
//				System.out.println(num_chambres);
			...
			if(  nombres_personnes.trim().length()==0 || dateArriver.trim().length()==0 || dateDepart.trim().length()==0 || date_reservation.trim().length()==0 || status.trim().length()==0 
				...
				try {
					...
					Connection con;
					 con =(Connection) db.db_connect();
						 PreparedStatement stmt = con.prepareStatement("UPDATE reservation SET =?, dateArriver=?, dateDepart= ?, date_reservation=?, status= ?, id_client=?, id_categorie= ?, "
						 		+ "num_chambres=? where id_reservation=? ");
		...
		btnModifier.setBounds(452, 685, 131, 41);
		...
		lblStatut.setFont(new Font("Tahoma", Font.BOLD, 17));
		...
		btnAnnuler.setBounds(690, 685, 131, 41);
		...
		btnNew.setBounds(206, 685, 141, 41);
		contentPane.add(btnNew);
		
		JLabel lblNewLabel_1 = new JLabel("New label");
		lblNewLabel_1.setIcon(new ImageIcon("C:\\Users\\User\\eclipse-workspace\\APP Bureau\\img\\25.jpg"));
		lblNewLabel_1.setBounds(0, 0, 1299, 766);
